...
import tqdm
import cv2
from CustomDataLoader import FaceDataset
...
def sample_plot_image(epoch):
    ...
    cv2.imwrite(f"./checkpoints/epoch_{epoch}.png", np.hstack(denoised_images)[:, :, ::-1])
...
for epoch in range(N_EPOCHS + 1):
    ...
    torch.save(model, './checkpoints/face_model.pth')
